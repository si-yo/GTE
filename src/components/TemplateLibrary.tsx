...
export const TemplateLibrary: React.FC<Props> = ({ isOpen, onClose, onOpenPlugins }) => {
  ...
  const loadTemplateToProject = (template: Template, graphId: string) => {
    // Create a mapping from template node IDs to real node IDs
  ...
  const confirmLoad = (mode: 'new' | 'replace') => {
    ...
    if (mode === 'new') {
      // Create new project with the template
      createNewProject(pendingTemplate.name);
      // Wait for project to be created then load template
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-gray-900 rounded-xl shadow-2xl w-[95vw] max-w-7xl h-[90vh] flex flex-col overflow-hidden border border-gray-700">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-700 bg-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            <span className="text-2xl sm:text-3xl">📚</span>
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">Bibliothèque de Templates</h2>
              <p className="text-xs sm:text-sm text-gray-400 hidden sm:block">30 exemples du plus simple au plus complexe</p>
            </div>
          </div>
